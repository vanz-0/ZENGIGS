...
# Video encoding settings - H.264 at 10Mbps, 30fps
...
def check_hardware_encoder_available() -> bool:
    """Always use software encoding on this machine (no NVIDIA GPU)."""
    return False
...
def get_cached_encoder_args() -> list[str]:
    ...
    if _hardware_encoder_available is None:
        ...
        if _hardware_encoder_available:
            print("[GPU] Hardware encoding enabled (h264_nvenc)")
        else:
            print("[CPU] Using software encoding (libx264)")

    if _hardware_encoder_available:
        return ["-c:v", HARDWARE_ENCODER, "-b:v", HARDWARE_BITRATE, "-r", str(TARGET_FPS)]
    else:
        return ["-c:v", SOFTWARE_ENCODER, "-preset", "fast", "-crf", SOFTWARE_CRF, "-r", str(TARGET_FPS)]
...
def get_speech_timestamps_silero(audio_path: str, min_speech_duration: float = 0.25, min_silence_duration: float = 0.5):
    ...
    Uses soundfile to read audio (avoids torchaudio/torchcodec DLL issues on Windows).
    """
    import torch
    ...
    # Use soundfile instead of torchaudio's read_audio to avoid torchcodec DLL issues on Windows
...
def concatenate_singlepass(input_path: str, segments: list, output_path: str):
    ...
    try:
        ...
        print("    Running FFmpeg trim+concat...")
        result = subprocess.run(cmd, capture_output=True, text=True)
...
def main():
    ...
    print("[JUMP-CUT] Jump Cut Editor (Single-Pass, trim+concat)")
    ...
    print(f"[DUR] Video duration: {duration:.2f}s ({duration/60:.1f} min)")

    # Extract audio for VAD
    print("[AUD] Extracting audio...")
    ...
    print(f"[MERGE] After merging: {len(speech_segments)} segments")

    # Add padding
    padding_s = args.padding / 1000
    speech_segments = add_padding(speech_segments, padding_s, duration)
    print(f"[PAD] After padding: {len(speech_segments)} segments")
    ...
    if args.keep_start and speech_segments and speech_segments[0][0] > 0:
        ...
        print("[START] Extended first segment to start at 0:00")

    # Calculate expected output duration
    total_speech = sum(end - start for start, end in speech_segments)
    print(f"[EST] Expected output: {total_speech:.1f}s ({total_speech/60:.1f} min)")
    print()
    ...
    if success:
        ...
        print(f"   Output   : {new_duration:.2f}s ({new_duration/60:.1f} min)")
        print(f"   Removed  : {removed:.2f}s ({100*removed/duration:.1f}%)")
        print(f"   Time     : {overall_time:.1f}s")
